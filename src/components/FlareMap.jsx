...
const FlareMap = ({flares, width, height, mode = 'globe'}) => {
    ...
    useEffect(() => {
        if (!mapData || !svgRef.current || !flares) return;
        ...
        if (mode === 'globe') {
            ...
            const heatmapData = flares.map(f => {
                ...
                const projectedCoords = coords ? projection([coords.lon, coords.lat]) : null;
                return {
                    ...f,
        ...
        const drag = d3.drag()
            ...
            .on('drag', (event) => {
                if (lastPosition) {
                    ...
                    const dx = event.x - x;
                    const dy = event.y - y;
                    setRotation(prevRotation => [prevRotation[0] + dx * 0.2, prevRotation[1] - dy * 0.2]);
                    setLastPosition([event.x, event.y]);
                }
            })
            .on('end', () => {
